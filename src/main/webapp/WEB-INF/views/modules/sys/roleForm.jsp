...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			// 用户-机构
			var zNodes2=[
					<c:forEach items="${privilegeList}" var="privilege">{id:"${privilege.id}", pId:"${not empty privilege.parent?privilege.parent.id:0}", name:"${privilege.name}"},
		            </c:forEach>];
			// 初始化树结构
			var tree2 = $.fn.zTree.init($("#privilegeTree"), setting, zNodes2);
			// 不选择父节点
			tree2.setting.check.chkboxType = { "Y" : "ps", "N" : "s" };
			// 默认选择节点
			var ids2 = "${role.menu1Ids}".split(",");
		...
		<%-- <div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="office" name="office.id" value="${role.office.id}" labelName="office.name" labelValue="${role.office.name}"
					title="机构" url="/sys/dept/treeData" cssClass="required"/>
			</div>
		</div> --%>
		<div class="control-group">
...
<%-- 		 <div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<div id="privilegeTree" class="ztree" style="margin-left:100px;margin-top:3px;float:left;"></div>
				<form:hidden path="menu1Ids"/>
			</div>
		</div>
		<div class="control-group">
