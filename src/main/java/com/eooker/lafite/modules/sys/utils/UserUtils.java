...
import com.eooker.lafite.common.utils.SpringContextHolder;
import com.eooker.lafite.modules.sys.dao.MenuDao;
import com.eooker.lafite.modules.sys.dao.OfficeDao;
...
import com.eooker.lafite.modules.sys.security.SystemAuthorizingRealm.Principal;
...
//	private static SysUsersDao sysUsersDao =SpringContextHolder.getBean(SysUsersDao.class);
	...
	public static final String CACHE_PRIVILEGE_LIST = "privileteList";
...
//	public static SysUsers getSysUsers(String id){
//		SysUsers user=sysUsersDao.get(id);
//		
//			
//			if (user == null){
//				return null;
//			}
//			user.setRoleList(roleDao.findList1(new Role(user)));
//		
//		return user;
//	}
	/**
...
//		User user =  userDao.getByNumber(temp);
...
//	public static SysUsers getUserByLoginName(String username){
//		SysUsers user = sysUsersDao.getByLoginName(new SysUsers(null, username));
//			if (user == null){
//				return null;
//			}
//			user.setRoleList(roleDao.findList1(new Role(user)));
...
//		if (principal!=null){
////			User user = get(principal.getId());
//			SysUsers user=sysUsersDao.get(principal.getId());
//			if (user != null){
//				return user;
//			}
//			return new SysUsers();
...
//			SysUsers user=getSysUser();
	...
	public static List<Privilege> getPrivilegeList(){
		@SuppressWarnings("unchecked")
		List<Privilege> privilegeList = (List<Privilege>)getCache(CACHE_PRIVILEGE_LIST);

		if (privilegeList == null){
			User user = getUser();
			if (user.isAdmin()){
				privilegeList = privilegeDao.findAllList(new Privilege());
			}
			else
			{
				Privilege m = new Privilege();
				m.setUserId(user.getId());
				privilegeList = privilegeDao.findByUserId(m);
			}
			putCache(CACHE_PRIVILEGE_LIST, privilegeList);
		}
